...
void display_graphic_16x16(u8 page,u8 column,u8 NUM)
{
	u16 i,j;
	for(j=0;j<2;j++)
	{
		lcd_address(page+(j+1)%2,column);
		for (i=0;i<16;i++)
		{
			WriteData(USER_MY1[NUM][i*2+j],1);
		}
...
void display_graphic_16x32(u8  page,u8 column,u8 NUM)
{
	u16 i,j;
	for(j=0;j<4;j++)
	{
		 if(j>=2)
		{
			lcd_address(page+(j+1)%2,column+15);
		}
		else
		{
			lcd_address(page+(j+1)%2,column);
		...
		for (i=0;i<=16;i++)
		{
		   if(j>=2)
		   	{
			   WriteData(USER_MY1[NUM][i*2+j+28],1);
